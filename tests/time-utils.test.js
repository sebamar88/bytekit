...
describe("TimeUtils", () => {
    it("sleep should wait for specified time", async () => {
        ...
        assert.ok(duration >= 45, `Duration ${duration}ms should be at least 45ms`);
    });
    ...
    it("parseDuration should throw on invalid format", () => {
        assert.throws(() => TimeUtils.parseDuration("invalid"), /Invalid duration format/);
        assert.throws(() => TimeUtils.parseDuration("100x"), /Invalid duration format/);
    });
