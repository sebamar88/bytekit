...
import { Logger, createLogger, consoleTransportNode, consoleTransportBrowser } from "../dist/utils/core/Logger.js";
...
test("Logger.child creates nested namespaces", () => {
    let lastNamespace = "";
    const transport = (entry) => { lastNamespace = entry.namespace; };
...
test("Logger handles transport failures", async () => {
    ...
    console.error = (msg) => { 
        if (typeof msg === 'string' && msg.includes("[Logger] Transport failure")) {
            errorLogged = true; 
        }
    };
