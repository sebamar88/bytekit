...
import { PaginationHelper } from "../dist/utils/helpers/PaginationHelper.js";
...
test("PaginationHelper throws when using wrong mode methods", () => {
    ...
    assert.throws(
        () => offsetPaginator.getCurrentPageByCursor(),
        /Use getCurrentPage\(\) in offset mode/
    );
    assert.throws(
        () => offsetPaginator.nextByCursor(),
        /Use next\(\) in offset mode/
    );
    assert.throws(
        () => offsetPaginator.previousByCursor(),
        /Use previous\(\) in offset mode/
    );
    assert.throws(
        () => offsetPaginator.goToCursor("abc"),
        /Use goToPage\(\) in offset mode/
    );

    assert.throws(
        () => cursorPaginator.getCurrentPage(),
        /Use getCurrentPageByCursor\(\) in cursor mode/
    );
    assert.throws(
        () => cursorPaginator.next(),
        /Use nextByCursor\(\) in cursor mode/
    );
    assert.throws(
        () => cursorPaginator.previous(),
        /Use previousByCursor\(\) in cursor mode/
    );
    assert.throws(
        () => cursorPaginator.goToPage(1),
        /Use goToCursor\(\) in cursor mode/
    );
});
