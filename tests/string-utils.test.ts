...
describe("camelCase", () => {
    ...
    it("convierte snake_case", () =>
        expect(camelCase("foo_bar_baz")).toBe("fooBarBaz"));
    it("convierte PascalCase", () =>
        expect(camelCase("FooBarBaz")).toBe("fooBarBaz"));
    it("convierte espacios", () =>
        expect(camelCase("hello world")).toBe("helloWorld"));
    it("string vacío", () => expect(camelCase("")).toBe(""));
    it("normaliza ñ en entrada", () =>
        expect(camelCase("niño_feliz")).toBe("ninoFeliz"));
    it("normaliza ü en entrada (alemán)", () =>
        expect(camelCase("über_cool")).toBe("uberCool"));
    it("normaliza é en entrada (francés)", () =>
        expect(camelCase("café_latte")).toBe("cafeLatte"));
});
...
describe("snakeCase", () => {
    ...
    it("normaliza ñ (español)", () =>
        expect(snakeCase("año nuevo")).toBe("ano_nuevo"));
    it("normaliza ö (alemán)", () =>
        expect(snakeCase("schön gut")).toBe("schon_gut"));
    it("normaliza è (francés)", () =>
        expect(snakeCase("très bien")).toBe("tres_bien"));
});
...
describe("pascalCase", () => {
    ...
    it("normaliza ü (alemán)", () =>
        expect(pascalCase("über_mensch")).toBe("UberMensch"));
    it("normaliza ñ (español)", () =>
        expect(pascalCase("niño_lindo")).toBe("NinoLindo"));
    it("normaliza ç (francés)", () =>
        expect(pascalCase("garçon_chic")).toBe("GarconChic"));
});
...
describe("truncate", () => {
    ...
    it("exactamente el límite", () => expect(truncate("abc", 3)).toBe("abc"));
...
describe("slugify", () => {
    ...
    it("solo caracteres inválidos", () => expect(slugify("!@#$%")).toBe(""));
});
