...
test("StringUtils.slugify with various options", () => {
    ...
    assert.equal(
        StringUtils.slugify("Hello World!", { separator: "_" }),
        "hello_world"
    );
...
test("StringUtils.truncate with options", () => {
    ...
    assert.equal(
        StringUtils.truncate(text, 10, { respectWordBoundaries: false }),
        "The quick…"
    );
...
test("StringUtils.mask", () => {
    ...
    assert.equal(
        StringUtils.mask("123", { visibleStart: 2, visibleEnd: 2 }),
        "123"
    );
...
test("StringUtils.interpolate with nested paths", () => {
    const template =
        "Hello {{ user.name }}, welcome to {{ user.settings.theme }} mode!";
...
test("StringUtils.interpolate edge cases", () => {
    ...
    assert.throws(
        () => StringUtils.interpolate(template, params, { strict: true }),
        /Missing value for key "missing"/
    );
...
test("StringUtils.initials", () => {
    ...
    assert.equal(
        StringUtils.initials("John Fitzgerald Kennedy", { limit: 3 }),
        "JFK"
    );
});
...
test("StringUtils.toQueryString complex scenarios", () => {
    ...
    assert.equal(
        StringUtils.toQueryString(params, { arrayFormat: "bracket" }),
        "a=1&b%5B%5D=2&b%5B%5D=3&c%5Bd%5D=4&f="
    );
    assert.equal(
        StringUtils.toQueryString(params, { arrayFormat: "comma" }),
        "a=1&b=2%2C3&c%5Bd%5D=4&f="
    );
    assert.equal(
        StringUtils.toQueryString(params, { skipEmptyString: true }),
