...
test("Validator.isLocalPhone for different locales", () => {
    ...
    assert.equal(Validator.isLocalPhone("12345678", "unknown-locale", { fallbackToGeneric: false }), false);
});

test("Validator.isCbu validates checksums", () => {
    // A valid CBU (example)
    const validCbu = "0070000010000000054321"; 
...
test("Validator.isStrongPassword options", () => {
    ...
    assert.equal(Validator.isStrongPassword("nonumbers!", { requireNumber: true }), false);
    assert.equal(Validator.isStrongPassword("NOSP3CIAL123", { requireSpecial: true }), false);
    assert.equal(Validator.isStrongPassword("nouppercase123!", { requireUppercase: true }), false);
});
