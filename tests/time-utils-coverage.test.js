...
test("TimeUtils.debounce logic", async () => {
    let calls = 0;
    const debounced = TimeUtils.debounce(() => { calls++; }, 50);
...
test("TimeUtils.debounce with maxWait", async () => {
    let calls = 0;
    const debounced = TimeUtils.debounce(() => { calls++; }, 100, { maxWait: 150 });
...
test("TimeUtils.throttle logic", async () => {
    let calls = 0;
    const throttled = TimeUtils.throttle(() => { calls++; }, 50);
...
test("TimeUtils measure helpers", async () => {
    ...
    assert.ok(resAsync.durationMs >= 10);
});
